...
# Setting eval_device as train_device
...
HP_ARCH = hp.HParam('architecture', hp.Discrete(['1d_conv'])) # iterate over training architectures. 
...
if args.held_out_num_groups is None:
  if args.dataset_name.find('realworld') != -1:
    ...
    num_users = 6
  HP_HELD_OUT = hp.HParam('held_out', hp.Discrete([i for i in range(num_users)])) 
else:
  print("PERFORMING HELD_OUT GROUP EVALUATION")
  HP_HELD_OUT = hp.HParam('held_out', hp.Discrete([i for i in range(args.held_out_num_groups)]))
# HP_EVAL_DEVICE = hp.HParam('eval_device', hp.Discrete(['head', 'chest']))
HP_EVAL_DEVICE = hp.HParam('eval_device', hp.Discrete([args.eval_device]))
...
if args.training_mode == 'multi':
    HP_TEMPERATURE = hp.HParam('contrastive_temperature', hp.Discrete([0.05]))
    HP_FINETUNE_TAKE = hp.HParam('fine_tune_take', hp.Discrete([0.1,0.25,0.5,0.75,1.0])) #percentage of the dataset used for finetuning
    HP_AUG = hp.HParam('augmentations', hp.Discrete(['none']))
    HP_MULTI_SAMPLING = hp.HParam('multi_sampling_mode', hp.Discrete(['unsync_neg']))
    HP_DYNAMIC_DEVICE_SELECTION = hp.HParam('dynamic_device_selection', hp.Discrete([1]))
    # HP_DEVICE_SELECTION_STRATEGY = hp.HParam('dev_sel_strategy', hp.Discrete(['random_selection','closest_pos_all_neg','closest_only']))  # ',
    HP_DEVICE_SELECTION_STRATEGY = hp.HParam('dev_sel_strategy', hp.Discrete(['closest_pos_all_neg']))  # ',
    # HP_WEIGHTED_COLLOSSL = hp.HParam('weighted_collossl', hp.Discrete([False, True])) 
    HP_WEIGHTED_COLLOSSL = hp.HParam('weighted_collossl', hp.Discrete([True])) 
    # HP_NEG_SAMPLE_SIZE = hp.HParam('neg_sample_size', hp.Discrete([1,5,10]))
    HP_NEG_SAMPLE_SIZE = hp.HParam('neg_sample_size', hp.Discrete([1]))
else: #supervised
    HP_LR = hp.HParam('learning_rate', hp.Discrete([1e-4]))
    HP_TAKE = hp.HParam('take', hp.Discrete([0.1,0.25,0.5,0.75,1.0]))
    HP_TEMPERATURE = hp.HParam('contrastive_temperature', hp.Discrete([-1.]))
    HP_FINETUNE_TAKE = hp.HParam('fine_tune_take', hp.Discrete([-1.0])) #percentage of the dataset used for finetuning
    HP_AUG = hp.HParam('augmentations', hp.Discrete(['none'])) #add various augmentations. TODO: not implemented yet 
...
if args.training_mode == 'multi':
  if args.train_device == 'thigh':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
      # "(['forearm', 'upperarm', 'head'], ['shin', 'chest', 'waist'])", 
      # "(['chest'], ['shin', 'upperarm', 'head', 'forearm', 'waist'])",
      # "(['shin'], ['chest', 'upperarm', 'head', 'forearm', 'waist'])",
      "([], [])"
    ]))
  elif args.train_device == 'chest':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
      # "(['waist', 'upperarm', 'head'], ['shin', 'forearm', 'thigh'])", 
      # "(['head', 'waist'], ['thigh', 'shin', 'upperarm', 'forearm'])", 
      "([], [])"
    ]))
  elif args.train_device == 'head':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
      # "(['thigh', 'shin', 'waist'], ['upperarm', 'forearm', 'chest'])", 
      # "(['upperarm', 'forearm', 'chest'], ['thigh', 'shin', 'waist'])", 
      # "(['chest', 'waist'], ['thigh', 'shin', 'upperarm', 'forearm'])", 
      "([], [])"
    ]))
  elif args.train_device == 'forearm':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
     # "(['shin', 'waist', 'thigh'], ['head', 'upperarm', 'chest'])",
      # "(['waist'], ['shin', 'head', 'upperarm', 'chest', 'thigh'])",
      # "(['thigh'], ['head', 'shin', 'waist', 'upperarm', 'chest'])", 
      "([], [])"
    ]))
  elif args.train_device == 'shin':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
      # "(['upperarm', 'forearm', 'head'], ['chest', 'waist', 'thigh'])",
      # "(['chest'], ['head', 'forearm', 'waist', 'upperarm', 'thigh'])", 
      # "(['upperarm'], ['head', 'forearm', 'waist', 'thigh', 'chest'])", 
      "([], [])"
    ]))
  elif args.train_device == 'upperarm':
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
      # "(['forearm'], ['head','chest', 'waist', 'thigh','shin'])",
      # "(['forearm', 'thigh'], ['head', 'waist', 'shin', 'chest'])", 
      # "(['forearm', 'chest','head'], ['waist', 'thigh', 'chest'])", 
      "([], [])"
    ]))
  else:
    HP_POS_NEG_DEVICES = hp.HParam('positive_negative_devices', hp.Discrete([
  ...
  for eval_device in HP_EVAL_DEVICE.domain.values:
    ...
    for ft_take in HP_FINETUNE_TAKE.domain.values:
      ...
      with tf.summary.create_file_writer(working_directory + '/logs/hparam_tuning_' + start_time + "/" + run_name + "-" + str(ft_take)).as_default():
          ...
          args.fine_tune_device = eval_device
