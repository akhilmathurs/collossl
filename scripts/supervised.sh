...
for device in ${positions[@]}; do
  for type in ${types[@]}; do
    python3 ../hparam_tuning_mp.py --exp_name=${exp_name}  --working_directory ${working_directory} --dataset_path ${dataset_path} --gpu_device=0,1,2,3,4,5,6,7  --held_out_num_groups=${held_out_num_groups} --n_process_per_gpu=2 --training_mode=${type} --train_device=${device} --training_batch_size=512 --training_epochs 200 --dataset_name ${dataset_name}
  done
done
